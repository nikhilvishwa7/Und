    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/+4nzja42ELQwzOWVl"
                    )
                ]
            ]
        ),
...
async def next_page(bot, query):
    ...
    if settings["button"]:
        ...
        btn.insert(
            ...
            [
                InlineKeyboardButton(
                    "Sᴛᴀʀᴛ Bᴏᴛ", url=f"https://telegram.me/{temp.U_NAME}"
                ),
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                settings["is_shortlink"]
                and not settings["botpm"]
                and clicked not in PREMIUM_USER
            ):
                if clicked == typed:
                    temp.SHORT[clicked] = query.message.chat.id
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer(
                "Jᴀʟᴅɪ Yᴀʜᴀɴ Sᴇ Hᴀᴛʜᴏ...📢\nJᴏɪɴ Bᴀᴄᴋᴜᴘ Cʜᴀɴɴᴇʟ Fɪʀsᴛ 😒", show_alert=True
            )
            return
        ident, file_id = query.data.split("#")
        if file_id == "send_all":
        ...
        btn2 = [
            ...
            [
                InlineKeyboardButton(
                    "Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/+4nzja42ELQwzOWVl"
                )
            ],
        ]
        if query.from_user.id in ADMINS:
        ...
        btn2 = [
            ...
            [
                InlineKeyboardButton(
                    "Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/Action_Movies_Group"
                )
            ],
        ]
        if query.from_user.id in ADMINS:
